...
@click.command()
@click.option("--title", prompt="Title text", help="Search for title text")
def search_command(title):
    if cache.get("cli_user") is None:
        console.print(Panel.fit("User not logged in yet!", style="red"))
        return
    user = cache.get("cli_user")
    fav_links = FavLink.objects.filter(user=user, title__contains=title).order_by(
        "date"
    )
